 ...
 */
export function initCamera(initialPosition, opts) {
    const position = (initialPosition !== undefined) ? initialPosition : new THREE.Vector3(-30, 40, 30);
    const options = opts || {};
    const aspect = window.innerWidth / window.innerHeight;

    // If caller requested orthographic, build a conservative frustum that
    // is large enough for typical demo scenes. Caller can supply frustumSize/near/far.
    if (options.type === 'orthographic') {
        const frustumSize = (options.frustumSize !== undefined) ? options.frustumSize : 200;
        const left = -frustumSize * aspect / 2;
        const right = frustumSize * aspect / 2;
        const top = frustumSize / 2;
        const bottom = -frustumSize / 2;
        const near = (options.near !== undefined) ? options.near : 0.1;
        const far = (options.far !== undefined) ? options.far : 10000;

        const camera = new THREE.OrthographicCamera(left, right, top, bottom, near, far);
        camera.position.copy(position);
        camera.lookAt(new THREE.Vector3(0, 0, 0));
        // store frustumSize so callers can recompute on resize
    ...
    const near = (options.near !== undefined) ? options.near : 0.1;
    const far = (options.far !== undefined) ? options.far : 100000;
